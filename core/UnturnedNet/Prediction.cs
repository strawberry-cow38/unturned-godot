...
{
    /// <summary>
    /// The client-side reconciliation ledger: the per-seq prediction ring, the ack cursor, the dead
    /// zone, the snap threshold, and the correction accounting. Since mp-clientauth-foot (wire v9)
    /// this serves ONLY the headless flat demo walker (ClientPrediction below) -- the real shell
    /// client is client-authoritative on foot (PlayerAuthority.cs) and reconciles NOTHING: it streams
    /// its transform and the server adopts it. The C3 replay-record machinery this class used to carry
    /// (the full-input ring, CollectReplayWindow) is deleted with that stack.
    ///
    /// Convergence shape: predictions are recorded AFTER the tick's correction is applied, so each
    /// fresh authoritative sample measures the REMAINING error (replace semantics, no double-count).
    /// </summary>
    public sealed class PredictionReconciler
    {
        ...
        /// at zero and the walker keeps its own prediction. Real Unturned corrects only past
        /// errorToleranceDistance = 0.02 m (PlayerInput.cs:1817).</summary>
        ...
        /// <summary>One 50 Hz tick of smoothing: returns the position delta to apply to the local walker
        ...
        /// position grid swallowed each tick. Demo-walker only (see the class doc).</summary>
