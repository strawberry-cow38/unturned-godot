...
- **F5b (next):** flow-rate BOOST (a powered pump raises the effective supply cap of its downstream hose) + transformer consumers (refinery oil→gas, sluice water→dirty: a Consumer that DELETES its input + OUTPUTS a different fluid via its own Source/Passthrough port at a converted rate) + rain barrels (a Storage that fills from weather) when weather lands.
