...
|---|---|---|---|
| 2.1 | **Power `PowerScale` is SP-only** | `PowerNet.cs:50` scales output ports by `d.PowerScale`; `DeployableReplication.cs:522` does not scale at all. A generator mid-spin-up produces full 4000 W server-side and `4000 × _powerLevel` in SP; a wind turbine produces a flat 2500 W server-side vs `2500 × _windFactor` (0..2) in SP | **FIXED** — wind off the sim tick; generator output binary per strawberry |
