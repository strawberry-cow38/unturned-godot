...
namespace UnturnedNet.Tests
{
    ...
    public class PacketHeaderGoldenTests
    {
        ...
        public void KeepAliveDatagram_GoldenBytes()
        {
            ...
            Assert.That(ToHex(captured, capturedLen), Is.EqualTo("752F08000000000000002800"));   // byte[1]=0x2F = Version 47 (v47 server-authoritative conversations: CommandNpcTalk(57)/CommandNpcChoose(58)/CommandNpcClose(59)/CommandNpcTrade(60) + EventNpcState(45); no existing block layout moves, so ONLY the version byte changes here -- which is this golden's whole job). ⚠ 0x2E = Version 46 is tinyclaw's radiation dose, in flight on their branch when this landed; on merge this chain must read 2F -> 2E -> 2D with BOTH entries, because taking either side alone silently deletes the other's record. Before that 0x2D = Version 45 (v45 tires+carjack: CommandFitTire(55)/CommandCarjack(56) + the vehicle entity's popped-tire mask). Before that 0x2C = Version 44 (v44 one-shot gestures: EventPlayerGesture(44); no new COMMAND, so the command table is untouched). Before that 0x2B = Version 43 (v43 handcuffs: CommandArrestPlayer(52)/CommandUnlockArrest(53)/CommandStruggle(54)). Before that 0x2A = Version 42 (v42 gestures: the combat entity gains a gesture byte + CommandRequestGesture(51)). Before that 0x29 = Version 41 (v41 respray: the vehicle entity gains a paint colour + CommandPaintVehicle(50)). Before that 0x28 = v40 (forage), 0x27 = v39 (CommandGunUnload), and 0x26 = Version 38 (v38 lit appliances: the container entity gains a "this cooker is ON" bit beside v36's door bit, so a burning barbecue raises its lid and smokes on every screen rather than only inside its opener's panel). Before that 0x25 = Version 37 ((v37 prop doors: CommandToggleObjectDoor(47) + EventObjectDoorState(43) + a prop-door list on the SystemInteractables block -- ONLY the version byte moves here, which is this golden's whole job). Before that 0x24 = v36 (shared containers), 0x23 = v35 (mp-seats), 0x21 = v33 (oxygen).
        }
