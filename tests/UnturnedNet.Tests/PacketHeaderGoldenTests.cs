...
namespace UnturnedNet.Tests
{
    ...
    public class PacketHeaderGoldenTests
    {
        ...
        public void KeepAliveDatagram_GoldenBytes()
        {
            ...
            Assert.That(ToHex(captured, capturedLen), Is.EqualTo("752108000000000000002800"));   // byte[1]=0x21 = Version 33 (v33 oxygen: the owner vitals block gains a fifth normalized vital, server-owned) -- ONLY that byte moved from 0x20/32. Note what this golden CANNOT see, again: a new field inside the vitals block leaves no mark on a keepalive, so the version byte is the entire evidence the block changed shape at all. Previously 0x20 = Version 32 (v32 craft-cancel: CommandCraftCancel(44) refunds a queued job -- before it, the ONLY refund path was disconnecting) -- ONLY that byte moved from 0x1F/31. Previously 0x1F = Version 31 (v31 timed crafting: EventCraftQueue(41) carries the owner's pending jobs) -- ONLY that byte moved from 0x1E/30. Previously 0x1E = Version 30 (v30 freezing: Item.frozen behind a gate bit, StorageOpenedEvent carries the freezer dims, PlayerInventory grows a tenth page) -- ONLY that byte moved from 0x1D/29. Previously 0x1D = Version 29 (v29 fuel bar: StorageOpenedEvent gains CookerFuel + EventCookerState(40)) -- ONLY that byte moved from 0x1C/28, which is the whole point of this golden. Note what it CANNOT see, again: the new event id 40 leaves no mark on a keepalive, so the version byte is the entire evidence that this wave was bumped at all. (The comment here once read "0x17 = Version 23" while the literal said 0x1A -- an earlier bump re-goldened the hex and left the prose behind. Keep them together.)
        }
