...
namespace UnturnedNet.Tests
{
    ...
    public class PacketHeaderGoldenTests
    {
        ...
        public void KeepAliveDatagram_GoldenBytes()
        {
            ...
            Assert.That(ToHex(captured, capturedLen), Is.EqualTo("753508000000000000002800"));   // byte[1]=0x35 = Version 53 (v53: Accept gained the server name + cap; previously v52 DropItemCommand's look-orb Point). ⭐ DERIVED THE SAME WAY THE LINE BELOW DEMANDS: 52 -> 53 is 0x34 -> 0x35 at byte[1] and NOTHING ELSE MOVES, because v53 changes the ACCEPT payload and a keepalive carries no accept. If any other byte had moved, the framing changed and that is not what v53 claims. Previously "753408..." = Version 52 (v52: DropItemCommand gained the look-orb Point; previously v51 CommandQuickTransfer(63)). ⭐ DERIVED, NOT PASTED: byte[1] is the version, 51 -> 52 is 0x33 -> 0x34, and EVERY OTHER BYTE IS UNCHANGED from the v51 literal above it in this file's history. A keepalive carries no command payload, so a payload-only bump like this one must move exactly that byte and nothing else -- if a second byte moves, the FRAMING changed too and that is not what was claimed. Re-pasting whatever the code now emits would assert only that the code equals itself.
        }
