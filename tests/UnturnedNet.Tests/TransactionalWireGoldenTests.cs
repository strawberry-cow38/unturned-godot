...
namespace UnturnedNet.Tests
{
    ...
    public class TransactionalWireGoldenTests
    {
        ...
        public void PlaceDeployableCommand_GoldenBytes()
        {
            ...
            // Re-goldened for v39-41. The command grew {Page, X, Y} -- WHICH JAR to spend, so placing a
            // deployable held out of a crate stops spending a same-id twin from another page (5c5e1bbc).
            //
            // ⭐ DERIVED, NOT RE-BAKED. A golden regenerated by printing what the code emits passes against that
            // code by construction and proves nothing; the only re-bless worth having says what SHOULD change and
            // why. Here: three WriteUInt8 append 24 ZERO bits (the fixture leaves Page/X/Y default), and every bit
            // after the yaw was already zero -- the old last byte is 0x01, one set bit and the rest padding. So
            // whatever the yaw's exact bit offset, the result is the same twelve bytes with three zero bytes after
            // them. Nothing before the append can move, and if any of it has, this test is right to fail.
            //
