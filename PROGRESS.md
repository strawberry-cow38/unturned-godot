...
- [ ] **Muzzle flash / smoke — upgrade PENDING master (2026-07-08)** — source: the real muzzle flash is a ParticleSystem + a Light (`firstMuzzleEmitter.Emit(1)`, from `FindMuzzleEffectAsset`) = flash sprite + smoke puff. Our port has a SIMPLIFIED stand-in (OmniLight energy 5 — a bit strong, washes the frame — + an unshaded spark). Offered master a real flash-sprite+smoke particle upgrade; awaiting the call. (Surface decals = bullet IMPACT marks, a separate hit-effect system, not the muzzle.)
