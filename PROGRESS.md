...
- [ ] **Zombie follow-ups — TODO** — exact per-map outfits (LevelZombies tables, in map files); hats/gear attachment meshes (extract item Model + mount on the Skull bone); more Startle/Attack clips (rig has only Startle_0-1 / Attack_0-2, so crawler/sprinter can't use their unique anims); finer head mesh (so the face reads crisp); MEGA/SPIRIT/bosses (arena/beacon).
