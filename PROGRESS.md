...
- [x] **Storage crates (0d2ce09)** — `StorageCrate` (Node3D, group "crates") = bounded InteractableStorage: a world container with its own item grid. **F** opens the nearest within 2.5 m → its contents load into the player's STORAGE page (7) so the existing dashboard ("CRATE" grid) + TryDrag handle it; closing saves back. Completes the inventory's external-storage page. Verified `--invcrate`.
- [x] **Crates in loot (e423b76)** — `LootSpawner` also scatters a few loot-filled crates, so the real game has crates to find + open.
...
- [x] **Full-game integration verified** — `--demo` render shows it all together: first-person zombie gunfight in the rain, loot + crates in the world, HUD vitals, day/night + fog. The complete survival loop in one playable scene.
