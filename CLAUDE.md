...
`test.sh` prints `[SUITE] <name> | PASS/FAIL | ...` per suite then `[SUMMARY] TOTAL: P passed, F failed | first failure: <name>`; a failing suite lists the failed test names + a copy-pasteable `repro:` line, and names the *first* failure to debug. TRX artifacts land in `.testresults/`. Exit codes: 0 clean, 1 test failure, 2 infra/build failure. It's the layered runner from `docs/TESTING_PROPOSAL.md`: **L0** (engine-free `dotnet test`) and **L1** (batched in-engine tests) are LIVE. Default `./test.sh` runs L0+L1.

⚠ **There is no L2 — the visual-golden tier was REMOVED 2026-09-11** (strawberry: "scrap the whole goldens concept"). Over its life it produced twelve false alarms and not one caught visual regression: every red was a golden destroyed by a merge picking a side or by a cleanup dropping a legitimate re-bake, never a rendering bug. A gate that only ever cries wolf trains you to ignore it. `tests/visual/`, `tools/visual_tests.py` and the `--visual` flag are gone.

**Visual verification did NOT go with it, and matters more now.** Render it and look at it — see the xvfb/Vulkan/movie-maker section below. That loop is what actually catches visual bugs here, and it is now the only thing that does.
