...
namespace UnturnedGodot.Testing
{
    ...
    public class GunMagReload : GameTest
    {
        ...
        public override IEnumerable<Step> Run()
        {
            ...
            for (byte b = 0; b < PlayerInventory.OWNPAGES; b++) { var pg = p.Inventory.items[b]; for (int i = pg.getItemCount() - 1; i >= 0; i--) if (pg.getItem((byte)i)?.item?.id == 6) pg.removeItem((byte)i); }
            ...
            for (byte b = 0; b < PlayerInventory.OWNPAGES; b++) { var pg = p.Inventory.items[b]; for (int i = pg.getItemCount() - 1; i >= 0; i--) if (pg.getItem((byte)i)?.item?.id == 5004) pg.removeItem((byte)i); }
