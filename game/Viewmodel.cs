                ...
                if (hb >= 0 && !EmptyHands && !Fists)   // EmptyHands/Fists -> no weapon mesh; just the bare arms in the ready hold
                {
                    ...
                    if (_isScope && ironMesh != null)
                    {
                        ...
                        var lensShader = new Shader { Code =
                            ...
                            "void fragment() { vec2 p = (UV - vec2(0.5)) * 2.0; float a = atan(p.y, p.x) + 0.2618; float seg = 0.5235988; float dd = cos(floor(0.5 + a/seg) * seg - a) * length(p); if (dd > 0.95) discard; ALBEDO = texture(scope_tex, UV).rgb; }\n" };   // 12-gon mask ALIGNED to the scope ring: ring verts measured at k*30deg (phase 0), the SDF's verts default to 15+k*30, so +0.2618rad (15deg = seg/2) rotates the lens POINTS onto the ring's points (master: "points ... rotated different"). apothem 0.95, seg = 2pi/12
