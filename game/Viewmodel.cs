...
namespace UnturnedGodot
{
    ...
    public partial class Viewmodel : Node3D
    {
        ...
        static System.Collections.Generic.Dictionary<string, GunVisual> LoadExtraVisuals()
        {
            ...
            // default Magazine item (tools/extract_gun_mags.py) -- the bulk arsenal was missing its mag mesh entirely.
            string mgp = ProjectSettings.GlobalizePath("res://content/mags.tsv");
            if (System.IO.File.Exists(mgp))
                foreach (var line in System.IO.File.ReadAllLines(mgp))
                {
                    var c = line.Split('\t');
                    if (c.Length < 3 || !d.TryGetValue(c[0], out var gv)) continue;
