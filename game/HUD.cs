...
namespace UnturnedGodot
{
    ...
    public partial class HUD : CanvasLayer
    {
        ...
        public void HubTick(double delta)   // PERF: hub-ticked at 30 Hz (was a per-frame engine callback; see TickHub)
        {
            ...
            {
                _crosshair3p.Visible = Player != null && (Player.ThirdPersonActive || (Player.CannonAiming && !Player.GunsightActive));   // the gunsight has its own reticle   // centre crosshair: third person -- and while a cannon is being laid on the screen centre (the seated FP view has no other reticle)
            ...
            if (Player != null)
            {
                ...
                else if (gun)
                {
                    ...
                    _fireMode.Text = shell != null ? PlayerController.PluralAmmo(shell, Player.Ammo) : Player.FiremodeName;
                    _fireMode.Modulate = shell == null ? new Color(1f, 1f, 1f, 0.7f)
                                       : shell.Contains("Slug") ? new Color(0.62f, 1f, 0.66f, 0.95f)       // slug green
                                       : shell.Contains("Beanbag") ? new Color(0.9f, 0.9f, 0.92f, 0.95f)   // beanbag white-grey
                                       : new Color(1f, 0.92f, 0.7f, 0.9f);                                 // buckshot amber
