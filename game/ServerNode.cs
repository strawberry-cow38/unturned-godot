...
namespace UnturnedGodot
{
    ...
    public partial class ServerNode : Node
    {
        ...
        public override void _Ready()
        {
            ...
            _server.Session.PeerConnected += peer => Log.Print($"[SERVER] player {peer.PlayerId} '{peer.Name}' joined ({_server.Session.Peers.Count} online)");
            _server.Session.PeerDisconnected += (peer, reason) => Log.Print($"[SERVER] player {peer.PlayerId} left ({reason})");
