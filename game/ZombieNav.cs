    ...
    public static class ZombieNav
    {
        ...
        public static void BuildOrLoad(Node worldRoot, List<NavPocket> pockets, bool overlay = false, bool save = true, bool bakeIfMissing = true)
        {
            ...
            for (int i = 0; i < pockets.Count; i++)
            {
                ...
                if (nm == null && !bakeIfMissing) { GD.Print($"[zombienav] pocket {i}: no baked file -- run --bakenav to generate; skipped"); continue; }   // game LOADS only, never gens
                if (nm == null)
                {
                    ...
                    baked++;
                    if (save) { try { ResourceSaver.Save(nm, path); } catch (System.Exception e) { GD.Print($"[zombienav] save skip {i}: {e.Message}"); } }   // only the FULL-world bake saves the canonical mesh (terrain-only verify modes pass save:false so they don't pollute it)
                }
