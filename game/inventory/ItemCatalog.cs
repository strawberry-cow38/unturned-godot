...
namespace SDG.Unturned
{
    public static class ItemCatalog
    {
        public static void RegisterAll()
        {
            ...
            Add(9112, "Fluid Splitter", 2, 2, EItemType.GENERIC, EItemRarity.UNCOMMON, 0, 0, "Splits one hose into three. Each output carries the full flow; consumers draw only what they need.");
            Add(9113, "Fluid Combiner", 2, 2, EItemType.GENERIC, EItemRarity.UNCOMMON, 0, 0, "Merges three hoses into one output -- their flows add together, and the load splits back across the sources.");
        ...
        public const int ThrowableStackSize = 5;   // every throwable, one number -- see WireStackableWood
        ...
        static void WireStackableWood()
        {
            ...
            // THROWABLES stack to 5 (strawberry 2026-09-14: "make frag grenades, smokes, flares stack to 5").
            // Driven off the throwable TABLE rather than a hand-written id list: the table already knows which
            // ids are throwables -- both grenades, flares 255-260, smoke 261-268 -- so a throwable added later
        ...
        static void WireShotgunShells()
        {
            ...
            Shell(5004, 1, 1);
            { var fmj = SDG.Unturned.Assets.find(5004); if (fmj != null) { fmj.stackSize = 128; fmj.ammoType = "FMJ"; fmj.magRound = "5.56x45mm NATO"; } }   // 5.56 FMJ: bullet type FMJ + CARTRIDGE (matched against a mag's accepted rounds when drag-loading); caliber set by Shell() above. 128 (strawberry 2026-09-08) -- also the quarter-bands the dropped bundle's round count reads off, so it wants to stay a multiple of 4
        ...
        static void DeriveMagazinesFromGuns()
        {
            ...
            foreach (var dat in System.IO.Directory.GetFiles(dir, "*.dat"))
            {
                ...
                if (g == null) continue;
                calibered += NameTheCaliber(g);   // BEFORE the shell/magazine gates below -- a shotgun has a caliber too
                if (g.ShellReload || g.MagazineId <= 0) continue;   // shell guns feed loose rounds
        ...
        /// caliber"). Retail's text names the round after the gun -- "chambered in Timberwolf ammunition" -- which
        /// says nothing a player can use to pick a magazine or compare two rifles. The .dat already carries
        /// Caliber_Name (".338 Lapua Magnum"), and GunDef has read it since the per-caliber damage rebalance.
        ///
        /// An OVERLAY applied at load, deliberately, for the same reason PlayerDamageByCaliber is one: the .tsv is
        /// GENERATED by tools/gen_item_catalog.py from a retail install on a Windows box, so it cannot be
        /// regenerated here and hand-editing it would be silently reverted by the next run. The ripped text stays
        ...
        ///
        /// The prefix is OPTIONAL (.*? not .+?) for the guns whose ammunition word is the whole phrase -- "chambered
        /// in Arrows.". Every bow passes the already-named guard first, because its caliber "Arrow" is a substring of
        /// "Arrows"; so does the Rocket Launcher and the Paintball Gun. Exactly one gun reaches the rewrite through
        /// that door, and it is a real content error rather than an edge case: the CROSSBOW's caliber is "Bolt" and
        /// its retail description says it fires arrows.</summary>
        static readonly System.Text.RegularExpressions.Regex ChamberedIn = new(
            @"chambered in\s+(.*?)\s*(ammunition|shells|arrows|rockets|missiles|paintballs)\b",
            System.Text.RegularExpressions.RegexOptions.IgnoreCase | System.Text.RegularExpressions.RegexOptions.Singleline);

        static int NameTheCaliber(UnturnedGodot.GunDef g)
        {
            if (string.IsNullOrWhiteSpace(g.CaliberName) || string.IsNullOrEmpty(g.Id)) return 0;
            if (!ushort.TryParse(g.Id, out var id)) return 0;
            var a = SDG.Unturned.Assets.find(id);
            if (a == null || string.IsNullOrEmpty(a.description)) return 0;
            string cal = g.CaliberName.Trim().Trim('"');
            if (cal.Length == 0) return 0;
            if (a.description.IndexOf(cal, System.StringComparison.OrdinalIgnoreCase) >= 0) return 0;   // already named
            var next = ChamberedIn.Replace(a.description, "chambered in " + cal, 1);
            if (next == a.description) return 0;
            a.description = next;
            return 1;
        }
        ...
        static void WireMagazines()
        {
            ...
            Text(1021, "German pistol chambered in .45 ACP.");   // runs AFTER the caliber pass, so it names the
                                                                //  cartridge itself. The point the old text made --
