...
namespace UnturnedGodot
{
    ...
    public partial class Vehicle : VehicleBody3D, ITowNode
    {
        ...
        // TANK HATCHES (master 2026-09-05): the model ships both hatches OPEN -- the turret's top lid standing on the roof
        ...
        public bool HasPeriscope => _driverHatch != null;
        /// <summary>Tank seats look out through FIXED optics, not from their heads: the driver through the visor window
        /// (a periscope), the gunner through the sight on the mantlet -- or from the cupola with the lid open. Returns the
        /// hull-local eye for PositionVehicleCam, or false for a vehicle/seat without one. The gunner's points live in
        /// the TURRET frame and are converted, so they turn with the turret.</summary>
        public bool SeatEyeOverride(int seat, out Vector3 local)
        {
            local = Vector3.Zero;
            if (seat == 0 && _driverHatch != null) { local = TankPeriscopeEye; return true; }
            if (seat == 1 && _turretHatch != null && TurretPivot != null) { local = ToLocal(TurretPivot.ToGlobal(GunnerHeadOut ? TankCupolaEye : TankSightEye)); return true; }
            return false;
        }
        /// <summary>Hinge the two peeled hatches toward their targets (120 deg/s -- a heavy steel lid, not a car door).
        /// Driver's visor: OPEN while the driver's seat is taken. Turret lid: OPEN while the gunner is aboard with his head
        /// out; a gunner who leaves is no longer standing in it.</summary>
        ...
        // Dinky Trailer: car-scale open trailer, 1 axle(s). Derived in notes/CAR_TRAILER_REPORT.md.
        static readonly Spec _dinky_trailer = new()
        {
            Mass = 300.000000f,
            Body = "dinky_trailer_body.txt", Wheel = "jeep_wheel.txt", WheelTex = "jeep_wheel_albedo.png", Palette = "car_trailer_palette.png",
            ...
            {
                (new Vector3(0.100000f, 0.219510f, 2.605695f), new Vector3(-0.499999f, -0.088676f, -2.006949f), -20.205981f),
                (new Vector3(0.100000f, 0.219510f, 2.605695f), new Vector3(0.499999f, -0.088676f, -2.006949f), 20.205981f),
        ...
        public static Vehicle BuildDinkyTrailer(int variant = 0) => Build(_dinky_trailer, variant, "dinky_trailer");

        // Small Trailer: car-scale open trailer, 1 axle(s). Derived in notes/CAR_TRAILER_REPORT.md.
        static readonly Spec _small_trailer = new()
        {
            Mass = 300.000000f,
            Body = "small_trailer_body.txt", Wheel = "jeep_wheel.txt", WheelTex = "jeep_wheel_albedo.png", Palette = "car_trailer_palette.png",
            WheelRadius = 0.600000f, Engine = 0f, SteerMax = 0f, SteerMin = 0f, SpeedMax = 0f, SpeedMin = 0f, Brake = 0f,
            BoxSize = new Vector3(2.224994f, 0.250000f, 3.921415f), BoxCenter = new Vector3(0.000000f, -0.148431f, 0.000000f),   // the deck slab itself; the load space above it stays open
            ExtraBoxes = new (Vector3, Vector3)[]
            {
                (new Vector3(0.250000f, 1.000001f, 3.921415f), new Vector3(-0.987497f, 0.476570f, 0.000000f)),
                (new Vector3(0.250000f, 1.000001f, 3.921415f), new Vector3(0.987497f, 0.476570f, 0.000000f)),
                (new Vector3(1.724994f, 1.000001f, 0.250000f), new Vector3(0.000000f, 0.476570f, -1.835707f)),
                (new Vector3(1.724994f, 1.000001f, 0.250000f), new Vector3(0.000000f, 0.476570f, 1.835707f)),
                (new Vector3(0.200000f, 0.100000f, 0.300000f), new Vector3(0.000000f, -0.028921f, -3.771757f)),
            },
            HullBoxes = new (Vector3 size, Vector3 center, float yawDeg)[]
            {
                (new Vector3(0.100000f, 0.219510f, 2.811301f), new Vector3(-0.531249f, -0.088676f, -2.301055f), -20.021160f),
                (new Vector3(0.100000f, 0.219510f, 2.811301f), new Vector3(0.531249f, -0.088676f, -2.301055f), 20.021160f),
                (new Vector3(2.224994f, 0.250000f, 3.921415f), new Vector3(0.000000f, -0.148431f, 0.000000f), 0f),
            },
        ...
        public static Vehicle BuildSmallTrailer(int variant = 0) => Build(_small_trailer, variant, "small_trailer");

        // Medium Trailer: car-scale open trailer, 2 axle(s). Derived in notes/CAR_TRAILER_REPORT.md.
        static readonly Spec _medium_trailer = new()
        {
            Mass = 300.000000f,
        ...
        // Horsebox Trailer: car-scale open trailer, 2 axle(s). Derived in notes/CAR_TRAILER_REPORT.md.
        static readonly Spec _horsebox_trailer = new()
        {
            Mass = 300.000000f,
            Body = "horsebox_trailer_body.txt", Wheel = "jeep_wheel.txt", WheelTex = "jeep_wheel_albedo.png", Palette = "car_trailer_palette.png",
            WheelRadius = 0.600000f, Engine = 0f, SteerMax = 0f, SteerMin = 0f, SpeedMax = 0f, SpeedMin = 0f, Brake = 0f,
            BoxSize = new Vector3(3.100000f, 0.250000f, 6.012836f), BoxCenter = new Vector3(0.000000f, -0.148431f, 0.000000f),   // the deck slab itself; the load space above it stays open
            ExtraBoxes = new (Vector3, Vector3)[]
            {
        ...
        public static Vehicle BuildHorseboxTrailer(int variant = 0) => Build(_horsebox_trailer, variant, "horsebox_trailer");
        ...
        static readonly Spec _ship = new()
        {
            ...
            // WHEELHOUSE GLASS. Generated by tools/gen_vehicle_glass.py off the deckhouse, with --band: the
            // silhouette rule that finds a car's greenhouse has no signal on a box (it read the band as 19.9-21.9,
            // which is the SOLID box above the windows, and found nothing). The real openings measure y
            // 16.85..19.41 in ship_body.txt -- two 2.8 m lights per side plus a 14.8 m forward screen between the
            // pillars at x +-7.5 -- and the five panes it then produced land on those bounds to within 5 cm.
            GlassMesh = "ship_glass.txt", GlassTint = new Color(0.62f, 0.73f, 0.78f, 0.26f),   // same tint as the cars
            ...
            // THE WHEELHOUSE IS THE LEVEL WITH THE WINDOWS IN IT, which is NOT the level this seat was on.
            // Glazing her settled it: the openings are at y 16.85..19.41, so the room behind them has floor 16.85
            // (plates at 16.62/16.75/16.85) and ceiling 19.5. The band above it, 19.5..22, is a SOLID box -- the
            // outer wall runs y 19.50..22.00 unbroken at x=9.0 -- and that is where the seat sat, in a windowless
            // room looking at a bulkhead. "Move the driver's seat into the cabin" (strawberry 2026-09-05) is
            // exactly right, and the earlier "up in the superstructure" was satisfied one deck too high.
            //
            // The helm block stands on that floor at the forward screen (z 10.46..10.97), the seat a metre behind
            // it. Deliberately NOT the size of the room: you walk up to a console, not into a trigger.
            Helm = (new Vector3(0f, 17.40f, 11.45f), new Vector3(2.2f, 1.1f, 0.9f)),
            Seats = new[] { new Vector3(0f, 17.35f, 12.4f) },  // driver seat (index 0), on the wheelhouse floor (plate 16.85)
            DriverEye = new Vector3(0f, 18.50f, 11.9f),        // eye 1.15 m up -- inside the forward screen's own
                                                               // y-span (16.82..19.43), looking down the deck
        };
        ...
        static readonly Spec _tank = new()
        {
            ...
            // ONE turret mesh. tank_turret_1.txt is the prefab's Yaw/Model_1 -- the LOD1 of the same turret (26 verts, a solid
            // lid up to y 3.44), and drawing it alongside LOD0 put a plate over the hatch opening LOD0 models (master 2026-09-05
            // "are we maybe layering two LODs of the tank or something? lol"). We were.
            TurretMeshes = new[] { "tank_turret.txt" }, TurretYawPivot = new Vector3(0f, 0f, 0.85f),
            DriverEye = TankPeriscopeEye,   // the FP driver looks out of the visor window (SeatEyeOverride does the same; this keeps the fallback honest)
        ...
        /// <summary>The turret's top hatch LID, modelled swung open: a 1.27 m plate standing on the roof's rear edge (x -0.2..1.08,
        /// z 1.16..1.60), leaning back ~10 deg, 3.62 up to 4.75, with a hinge wedge reaching down to 3.26 at x 0.44. Its 20
        ...
        static void BuildTankExtras(Vehicle v, Spec s, Material bodyMat)
        {
            ...
            if (s.TurretMeshes != null)
            {
                ...
                foreach (var t in s.TurretMeshes)   // baked centred on the yaw pivot -> local 0
                {
                    ...
                    var (rest, _) = ContentProvider.ParseObjSplitByZone(path, new[] { TankMgBox, TankMgBarrel, TankTopHatch });
                    ...
                    var (_, lid) = ContentProvider.ParseObjSplitByZone(path, TankTopHatch.min, TankTopHatch.max);
        ...
        public static bool IsRoadVehicle(string name)
        {
            ...
            if (!known) foreach (var n in new[] { "off_roader", "vw_golf", "mini", "heli", "scout", "bird", "containership", "plane", "car_trailer", "horsebox" }) if (n == name) { known = true; break; }
        ...
        static Spec SpecFor(string name) => name switch
        {
            ...
            "golf" => _golf, "vw_golf" => _golf, "wagon" => _wagon, "dinky_trailer" => _dinky_trailer, "small_trailer" => _small_trailer, "medium_trailer" => _medium_trailer, "large_trailer" => _large_trailer, "horsebox_trailer" => _horsebox_trailer, "horsebox" => _horsebox_trailer, "car_trailer" => _dinky_trailer, "suv" => _wagon, "tank" => _tank,
