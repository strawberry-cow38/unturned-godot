#!/usr/bin/env python3
"""extract_trap_meshes.py -- rip the WORLD meshes + albedos for the trap deployables (landmine / spike / charge)
out of core.masterbundle into content/objects/, so the placed Deployables can drop their ProcBox gray placeholders
for the real models. VERBATIM reuse of extract_battery.py's LOD0 / world-transform / X-negate convention so the
meshes line up exactly like Generator_0 / Battery_0. Item prefab found by container path (probe_traps.py mapped them)."""
...
# (unique container-path substring -> output target name). Trailing slash on 'charge/' so charge_bounce/_precision don't match.
TRAPS = [
