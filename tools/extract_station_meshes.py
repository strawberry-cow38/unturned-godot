#!/usr/bin/env python3
"""extract_station_meshes.py -- rip the WORLD meshes + albedos for the CRAFTING-STATION barricades (workbench /
campfire / ovens / kiln / loom / sewing / spinning / chemistry) out of core.masterbundle into content/objects/, so
the station Deployables drop their ProcBox gray placeholders for the real models. VERBATIM reuse of
extract_trap_meshes.py's LOD0 / world-transform / X-negate convention so the meshes line up like Generator_0."""
import UnityPy, os, numpy as np
...
# (unique container-path substring -> output target name = the DeployableDef.Model). Trailing slash to avoid partial matches.
STATIONS = [
    ("items/barricades/workbench/", "Workbench_0"),
    ("items/barricades/campfire/", "Campfire_0"),
    ("items/barricades/chemistrylab/", "ChemistryLab_0"),
    ("items/barricades/kiln/", "Kiln_0"),
    ("items/barricades/loom/", "Loom_0"),
    ("items/barricades/oven_brick/", "Oven_Brick_0"),
    ("items/barricades/oven_electric/", "Oven_Electric_0"),
    ("items/barricades/sewingtable/", "SewingTable_0"),
    ("items/barricades/spinningwheel/", "SpinningWheel_0"),
]
...
def extract_one(path_sub, target):
    ...
    for gp in pick:
        ...
        for line in txt.splitlines():
            ...
            if p[0] == "v":
                ...
                Fs.append(idx)
    if not Vs:
        print(f"[{target}] NO GEOMETRY extracted"); return
    ...
    if best:
        ...
        print(f"[{target}] NO TEXTURE found")

for path_sub, target in STATIONS:
    extract_one(path_sub, target)
