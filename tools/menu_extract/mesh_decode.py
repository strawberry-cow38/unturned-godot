...
def decode(path, out_obj, flipz=True):
    ...
    stride = len(vbytes) // vcount   # authoritative (handles padding + float16 channels); position is always ch0 @ off0 f32
    pos = chans[0]
    ...
    def rd(ch, i, n):   # read n components by semantic (pos/normal=3, uv=2); ignore the parsed dim (can mis-parse)
        ...
        return struct.unpack_from('<'+code*n, vbytes, i*stride + off)
    verts, norms, uvs = [], [], []
    ...
    print(f"{out_obj}: {vcount}v {len(idx)//3}t stride={stride} uv={bool(uvs)} nrm={bool(norms)} bbox=({bb[0]:.3f},{bb[1]:.3f},{bb[2]:.3f})")
    return bb
