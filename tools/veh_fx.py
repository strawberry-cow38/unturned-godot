...
def main_tex(mat_obj):
    try:
        for e in mat_obj.read_typetree().get("m_SavedProperties",{}).get("m_TexEnvs",[]):
            ...
            if n=="_MainTex" and isinstance(te,dict):
                return te.get("m_Texture",{}).get("m_PathID")
    except: pass
    return None
# search a few vehicle prefabs for fire/smoke/exhaust nodes
found={}
for p,o in env.container.items():
    if "/vehicles/" in p and p.endswith("vehicle.prefab") and o.type.name=="GameObject":
        def walk(pid):
            go=by_id.get(pid)
            if not go: return
            tt=go.read_typetree(); nm=tt.get("m_Name","").lower()
            if nm in ("fire","smoke_0","smoke_1","exhaust") and nm not in found:
                for c in comps(tt):
                        ...
                        if mats:
                            ...
                            if tp and tp in by_id:
                                try:
                                    ...
                                    if img:
                                        img.save(os.path.join(OUT,f"veh_{nm}.png"))
                                        found[nm]=(by_id[tp].read().m_Name, img.size)
                                except Exception as e: found[nm]=("ERR",str(e))
...
print("missing:", [x for x in ("fire","smoke_0","smoke_1","exhaust") if x not in found])
