#!/usr/bin/env python3
"""extract_projectile.py -- rip the rocket launcher's projectile.prefab Model_0 (the visible flying rocket)
from core.masterbundle -> rocket_projectile.txt (mesh, Unturned->Godot: Z-neg + winding reverse) + rocket_projectile_albedo.png.
Same mesh/albedo pipeline as extract_gun.py, but the prefab root is 'Projectile' (not 'Item')."""
...
OUTDIR = sys.argv[1] if len(sys.argv) > 1 else r"C:\claude-workspace\gunout"
...
prefab = next((o for p, o in env.container.items()
               if "launcher_rocket/projectile.prefab" in p.lower() and o.type.name == "GameObject"), None)
if not prefab:
    print("NO projectile.prefab"); sys.exit(1)
...
for ch in tr.read_typetree().get("m_Children", []):
    ...
    if cgo and cgo.read_typetree().get("m_Name") == "Model_0":
        model0 = cgo
if not model0:
    print("NO Model_0 in projectile"); sys.exit(1)
...
L = ["# rocket projectile Model_0 -> Godot (Z negated + winding reversed)"]
...
for f in Fs:
    ...
    L.append(s)
open(os.path.join(OUTDIR, "rocket_projectile.txt"), "w").write("\n".join(L) + "\n")
alb = "NONE"
    ...
    if mo:
        for pair in mo.read_typetree().get("m_SavedProperties", {}).get("m_TexEnvs", []):
            ...
            if nm == "_MainTex" and isinstance(val, dict):
                ...
                if to:
                    to.read().image.convert("RGBA").save(os.path.join(OUTDIR, "rocket_projectile_albedo.png"))
                    alb = "rocket_projectile_albedo.png"
print("PROJECTILE verts", len(Vs), "tris", len(Fs), "albedo", alb)
