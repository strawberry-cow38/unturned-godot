#!/usr/bin/env python3
"""extract_consumable_anims.py <ItemName> -- rip a consumable's Equip/Use FP anim clips into rig.json as
Consume_Equip / Consume_Use (the Viewmodel plays these). Same clip->rig conversion as extract_gun_holds.py
(Unity->Godot quaternion, Skeleton-relative). Category-agnostic (food/medical/...)."""
...
def clips(name):
    for path, obj in cont.items():
        if f"/{name}/animations.prefab" in str(path).lower() and "items/" in str(path).lower() and obj.type.name == "GameObject":
            go = obj.read(); out = {}
...
added = []
for src, dst in [("Equip", "Consume_Equip"), ("Use", "Consume_Use")]:
    if src in cl:
        rig["anims"][dst] = convert(cl[src]); added.append(dst)
json.dump(rig, open(RIG, "w"))
print(f"{name}: added {added} (clips present: {list(cl.keys())}); rig.json bytes: {os.path.getsize(RIG)}")
