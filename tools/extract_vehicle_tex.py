...
for p, o in env.container.items():
    if o.type.name == "GameObject" and ("vehicles/"+sys.argv[1].lower()) in p.lower() and tt(o).get("m_Name", "").lower() == sys.argv[1].lower():
        root = o; break
if not root:
    for p, o in env.container.items():
        if o.type.name == "GameObject" and ("vehicles/"+sys.argv[1].lower()) in p.lower(): root = o; break
if not root:
    print("NO PREFAB"); raise SystemExit
...
if best:
    best[1].save(os.path.join(OUT, sys.argv[1].lower()+"_body_tex.png"))
