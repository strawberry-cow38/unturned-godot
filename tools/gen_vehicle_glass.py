...
verts, faces = [], []
def quad(a, b, c, d):
    i = len(verts) + 1; verts.extend([a, b, c, d])
...
for c in range(cur):
    ...
    for k in range(len(rows) - 1):
        ...
        for sx in (+1, -1):
            x = sx * (XS - INSET)
            quad(np.array([x, y1, a1]), np.array([x, y1, b1]),
                 np.array([x, y2, b2]), np.array([x, y2, a2]))

solid = g > 0
for zr, take_max in (((0.80, 1.80), True), ((-1.80, -0.60), False)):
    e = []
    ...
    quad(np.array([-XW, ylo, zlo]) + off, np.array([XW, ylo, zlo]) + off,
         np.array([XW, yhi, zhi]) + off, np.array([-XW, yhi, zhi]) + off)

with open(OUT, 'w') as fh:
    fh.write("# GENERATED by tools/gen_vehicle_glass.py -- do not hand-edit.\n")
    fh.write("# Window panes traced from the body's see-through apertures. Rendered translucent via\n")
    fh.write("# Vehicle.Spec.GlassMesh + GlassTint (Vehicle.AddGlassOverlay).\n")
    fh.write("g Glass\n")
    for v in verts: fh.write(f"v {v[0]:.6f} {v[1]:.6f} {v[2]:.6f}\n")
    for f in faces: fh.write(f"f {f[0]} {f[1]} {f[2]}\n")
print(f"{OUT}: {len(verts)} v, {len(faces)} f")
